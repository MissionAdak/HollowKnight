...
class Game:
	def __init__(self):
		...
		# Start with menu instead of level
		self.current_stage = Menu(self.switch_stage)
	...
	def switch_stage(self, target, unlock = 0):
		if target == 'level':
			...
			# Stop any existing music before playing new track
			pygame.mixer.stop()
			self.bg_music.play(-1)
			
		elif target == 'menu':
			# Stop any existing music before creating menu
			pygame.mixer.stop()
			self.current_stage = Menu(self.switch_stage)
			self.game_active = False
	...
	def import_assets(self):
		...
		self.bg_music = pygame.mixer.Sound(join('..', 'audio', 'ingamemusic.mp3'))
		self.bg_music.set_volume(0.5)

	def check_game_over(self):
		if self.game_active and self.data.health <= 0:
